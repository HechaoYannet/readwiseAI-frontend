...
export async function loginUser(loginId: string, password: string): Promise<LoginResponse> {
  if (!API_BASE) {
    ...
    if (loginId === 'admin' && password === 'admin') {
      return { user_id: 'admin_001', username: '管理员', access_token: `mock_admin_token_${Date.now()}`, token_type: 'bearer', role: 'admin' };
    }
    if (password.length >= 6) {
      return { user_id: `mock_${Date.now()}`, username: loginId, access_token: `mock_token_${Date.now()}`, token_type: 'bearer', role: 'user' };
    }
...
export async function getMe(token: string): Promise<UserProfile> {
  if (!API_BASE) {
    await sleep(300);
    return { id: 'mock_001', username: '张三', exam_region: '全国I卷', grade: '高三', school: '示范高中', role: 'user', status: 'active', created_at: new Date().toISOString(), last_login_at: new Date().toISOString() };
  }
...
export async function updateMe(token: string, payload: UpdateMePayload): Promise<UserProfile> {
  if (!API_BASE) {
    await sleep(500);
    return { id: 'mock_001', username: payload.username ?? '张三', exam_region: payload.exam_region ?? '全国I卷', grade: payload.grade ?? '高三', school: payload.school ?? '示范高中', role: 'user', status: 'active', created_at: new Date().toISOString(), last_login_at: new Date().toISOString() };
  }
  const res = await fetch(`${API_BASE}/api/users/me`, { method: 'PUT', headers: authHeaders(token), body: JSON.stringify(payload) });
...
export async function changePassword(token: string, payload: ChangePasswordPayload): Promise<{ message: string }> {
  ...
  const res = await fetch(`${API_BASE}/api/users/password`, { method: 'PUT', headers: authHeaders(token), body: JSON.stringify(payload) });
...
export async function addTrainingRecord(token: string, record: TrainingRecord): Promise<{ message: string; record: TrainingRecord }> {
  if (!API_BASE) {
    await sleep(300);
...
export async function addMistake(token: string, mistake: Omit<MistakeRecord, 'review_count' | 'next_review_at' | 'created_at'>): Promise<{ message: string; mistake_id: string }> {
  if (!API_BASE) {
    await sleep(300);
...
export async function updateMistake(token: string, mistakeId: string, updates: Partial<MistakeRecord>): Promise<{ message: string; mistake_id: string }> {
  if (!API_BASE) {
    await sleep(300);
...
export async function addPowerRecord(token: string, score: number, reason?: string): Promise<{ message: string; score: number }> {
  if (!API_BASE) {
    await sleep(300);
